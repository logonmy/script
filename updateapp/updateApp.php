<?php
	/*
	  ...
	  *			    ps:  第一次写php，发现此脚本写得很烂，占用内存太大，定时跑的时候，每个月
	  *					 总有那么几天要卡死一次，如果你发现此脚本跑的时间大于1个小时，就毫不
	  *					 犹豫的kill掉吧。或者重新写一下，逻辑不复杂。
	  * =====================================================================================
	*/
	...
	foreach( $app_names as $app_name )
    {
		...
		for( $i = 0; $i <= PAGE; $i++){
			$curr_page = 10 * $i;
			$content_url = "http://m.baidu.com/s?word=$app_name_tmp&st=10a081&tn=webmkt&pn=$curr_page&rn=10";
		...
		{	
			if( $redis -> sIsMember( $compare_redis, $id ) )
			{
			    $query = "update check_baidu_app set
						  ...
						  where app_id = '$id' and DATE(date) = DATE(now());";
	...
	function traverse_span_list( &$span_list )
	{
        ...
        for( $i = 0; $i < $span_list -> length; ++$i )
        {
			...
			if( !strcmp( $id_item, "appname" ) ){
				...
				if( !strcmp( $title_name, $app_name_tmp ) ){
					$title_flag = 1;
					break;
				}
			}
